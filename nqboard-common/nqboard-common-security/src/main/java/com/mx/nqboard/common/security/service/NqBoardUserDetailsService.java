package com.mx.nqboard.common.security.service;
 ...
 * @date 2025/05/31
 */
public interface NqBoardUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new NqBoardUser(info.getUserId(), info.getDept().getDeptId(), info.getUsername(),
	...
	/**
	 * 通过用户实体查询用户详情
	 * @param nqBoardUser 用户实体对象
	 * @return 用户详情信息
	 */
	default UserDetails loadUserByUser(NqBoardUser nqBoardUser) {
		return this.loadUserByUsername(nqBoardUser.getUsername());
	}
